    ...
    def _build_model(self):
        ...
        with tf.variable_scope("clockwork_cell") as scope:
            ...
            for time_step in range(1, self.config.num_steps + 1):
                ...
                # pred.append(temp2) #and this


                # print("Shape of Predictions at validation: " % np.shape(self.predictions))
                # self.predictions = tf.identity(self.predictions)

            # self.predictions = tf.concat(axis=1, values=pred, name="predictions")

            # Save the final hidden state
            self.final_state = self.state         # for full prediction
            ...
            # self.loss = tf.losses.mean_pairwise_squared_error(self.targets, self.predictions, weights=1.0)
            # print("shape of loss")
